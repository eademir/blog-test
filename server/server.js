...
mongodb.MongoClient.connect(dbUrl, (err, db) =>  {
    ...
    app.get('/api/blogs/:_id', (req, res) => {
        db.collection('blogs').findOne({ _id: new mongodb.ObjectId(req.params._id) }, (err, movie) => {
            res.json({ movie });
        })
    });

    app.delete('/api/blogs/:_id', (req, res) => {
        db.collection('blogs').deleteOne({ _id: new mongodb.ObjectId(req.params._id) }, (err, r) => {
            if (err) { res.status(500).json({ errors: { global: err }}); return; }
